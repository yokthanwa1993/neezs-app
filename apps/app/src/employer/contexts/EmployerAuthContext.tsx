...
import { employerAuth, employerDb } from '../lib/employerFirebase';
import { onAuthStateChanged, signOut, signInWithCustomToken } from 'firebase/auth';
import { buildCallbackUrl } from '@/shared/lib/codespaces';
import { useEmployerLiff } from './EmployerLiffContext';
...
interface EmployerUser extends User {
  role: 'employer';
  profile?: {
    lineUserId?: string;
    displayName?: string;
...
export const useEmployerAuth = () => {
  ...
  if (context === undefined) {
    throw new Error('useEmployerAuth must be used within a EmployerAuthProvider');
  }
  return context;
};
...
export const EmployerAuthProvider: React.FC<EmployerAuthProviderProps> = ({
  ...
  const { liff, isLiffReady, initializeLiff, liffError } = useEmployerLiff();

  // Network monitoring
  useEffect(() => {
    const handleOnline = () => {
      setIsOnline(true);
      logger.info('EmployerAuthProvider: Network connection restored');
    };

    const handleOffline = () => {
      setIsOnline(false);
      logger.warn('EmployerAuthProvider: Network connection lost');
    };
  ...
  // Start health monitoring (skip in development to reduce noise)
  useEffect(() => {
  ...
  useEffect(() => {
    if (user && !sessionStartTimeRef.current) {
      ...
      logger.info('EmployerAuthProvider: Session started', { userId: user.uid });
    }
  }, [user]);
  ...
  useEffect(() => {
    logger.info('EmployerAuthProvider: Setting up persistent auth state listener');
    const unsubscribe = onAuthStateChanged(employerAuth, (firebaseUser) => {
      logger.info('EmployerAuthProvider: Auth state changed', {
      ...
      try {
        ...
        if (!firebaseUser && bypass) {
          const mock: EmployerUser = {
            // @ts-expect-error allow minimal mock of Firebase User
            uid: `mock_employer_${Date.now()}`,
            email: 'mock_employer@dev.neeiz.com',
            providerId: 'custom',
            displayName: 'Mock ผู้จ้างงาน',
            role: 'employer',
            getIdToken: async () => 'mock-token',
          } as any;
          setUser(mock);
          setIsLoading(false);
          logger.info('EmployerAuthProvider: Bypass mode active, using mock employer user');
          return;
        }
      } catch {}

      if (firebaseUser) {
        const employerUser: EmployerUser = { ...firebaseUser, role: 'employer' };
        setUser(employerUser);
        logger.info('EmployerAuthProvider: User state set', { userId: employerUser.uid });
      } else {
        setUser(null);
        logger.info('EmployerAuthProvider: User state set to null');
      ...
      setIsLoading(false);
      logger.info('EmployerAuthProvider: Loading state set to false');
    });

    // Cleanup subscription on unmount
    return () => {
      logger.info('EmployerAuthProvider: Cleaning up auth state listener');
      unsubscribe();
    };
  }, [initializeLiff]);
    ...
    if (!user && isLiffReady) {
      ...
      try {
        ...
        if (until && Date.now() < until) {
          logger.info('EmployerAuthProvider: Suppressing LIFF auto-login due to recent logout');
          return;
        }
      } catch {}
      if (liff.isInClient() && liff.isLoggedIn()) {
        logger.info('EmployerAuthProvider: LIFF auto-login initiated');
        ...
        const performLiffAutoLogin = async () => {
          try {
            ...
            const response = await liffBreaker.current.execute(async () => {
              return await fetch('/api/auth/employer-liff-token', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ accessToken, role: 'employer' }),
              });
            });
            ...
            logger.info('EmployerAuthProvider: LIFF auto-login successful');
          } catch (error) {
            logger.error('EmployerAuthProvider: LIFF auto-login failed', { error: error.message });
  ...
  const login = useCallback(async () => {
    setIsLoading(true);
    logger.info('EmployerAuthProvider: Login initiated by user');

    try {
      // ตรวจสอบว่าเป็น LIFF environment หรือไม่
      if (liff && liff.isInClient()) {
        logger.info('EmployerAuthProvider: In LIFF, redirecting to LIFF login');
        await liffBreaker.current.execute(async () => {
          liff.login({ redirectUri: window.location.href });
        });
      } else {
        logger.info('EmployerAuthProvider: In web browser, requesting secure authorize URL (PKCE + state)');
        const redirectUri = (import.meta as any).env?.VITE_CODESPACES_BASE_DOMAIN
          ? buildCallbackUrl('employer', 5000)
          : (import.meta as any).env?.VITE_LINE_REDIRECT_URI_EMPLOYER || `${window.location.origin}/employer/auth/callback`;
        const resp = await fetch('/api/auth/authorize', {
          method: 'POST',
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ role: 'employer', redirectPath: '/employer/home', redirectUri })
        });
        if (!resp.ok) {
      ...
      logger.error('EmployerAuthProvider: Login failed with circuit breaker', { error: error.message });
  ...
  const logout = async () => {
    logger.info('EmployerAuthProvider: Logging out');
    ...
    try {
      ...
      try {
        if (lineUserId) {
          const payload = JSON.stringify({ lineUserId, message: 'ออกจากระบบ Neezs Employer แล้ว' });
          let notified = false;
      ...
      if (!suppressUntil) {
        suppressUntil = String(Date.now() + 60_000);
      }
      localStorage.clear();
      ...
      logger.info('EmployerAuthProvider: Logout successful');
      // No hard navigation; keep current view. LIFF window may close above when in client.
    } catch (error: any) {
      logger.error('EmployerAuthProvider: Logout failed', { error: error.message });
    ...
    const checkSessionTimeout = () => {
      ...
      if (lastActivity) {
        ...
        if (timeSinceLastActivity > sessionTimeout) {
          logger.info('EmployerAuthProvider: Session timeout detected, logging out');
          logout();
          return;
        }
      }
  ...
  const refreshToken = useCallback(async () => {
    ...
    try {
      logger.info('EmployerAuthProvider: Refreshing token');
      // Firebase automatically handles token refresh
      // This is just for explicit refresh if needed
      await firebaseBreaker.current.execute(async () => {
        await user.getIdToken(true);
      });
      logger.info('EmployerAuthProvider: Token refreshed successfully');
    } catch (error) {
      logger.error('EmployerAuthProvider: Token refresh failed', { error: error.message });
      throw error;
    }
  }, [user]);

  const clearSession = useCallback(() => {
    logger.info('EmployerAuthProvider: Clearing session');
  ...
  const retryLogin = useCallback(async (maxAttempts = 3) => {
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        logger.info(`EmployerAuthProvider: Retry login attempt ${attempt}/${maxAttempts}`);
        await login();
        return; // Success, exit retry loop
      } catch (error) {
        logger.warn(`EmployerAuthProvider: Retry login attempt ${attempt} failed`, { error: error.message });
