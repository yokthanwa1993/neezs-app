...
import { useJobs } from '@/shared/contexts/JobContext';
...
const EmployerJobSummary: React.FC = () => {
    ...
    const handlePostJob = async () => {
        ...
        // Local UI add for immediate feedback
        const localJob = {
        ...
        try {
            // Persist to backend so Seeker side can see it
            await apiClient.post('/api/jobs', {
                title: localJob.title,
                description: localJob.description,
                category: '',
                location: localJob.location,
                salary: Number(wage || 0),
                jobType: locationMode === 'online' ? 'freelance' : 'part-time',
                status: 'active',
                employerId: 'app-anonymous',
                images: Array.isArray(images) ? images : [],
            });
            // Also update local list for the employer UI
            addJob(localJob as any);
            alert('ประกาศงานเรียบร้อยแล้ว!');
            navigate('/employer/add-job', { replace: true });
        } catch (err) {
            console.error('Failed to post job:', err);
            alert('ไม่สามารถบันทึกงานไปยังระบบได้ กรุณาลองใหม่');
        }
    };
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-4 flex flex-col justify-center">
             <Card className="w-full max-w-mobile-lg mx-auto">
                <CardHeader>
