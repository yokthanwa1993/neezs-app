...
const EmployerLogin: React.FC = () => {
  const { login, isLoading } = useEmployerAuth();
  const { isMobile, deviceType } = useMobile();

  logger.info('EmployerLogin: Component rendered', { isLoading, isMobile, deviceType });

  const handleLogin = async () => {
    try {
      logger.info('EmployerLogin: Login button clicked');
      await login();
    } catch (error) {
      logger.error('EmployerLogin: Login failed', error);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 px-4 py-4">
      <MobileContainer className="max-w-mobile-lg">
        <div className="mobile-card text-center">
          ...
          <h1 className="text-mobile-2xl font-bold text-gray-900 mb-2">NEEZS</h1>
          <p className="text-mobile-sm text-gray-600 mb-6 sm:mb-8">สำหรับนายจ้าง</p>

          {isLoading ? (
            <div className="mobile-loading">
              <div className="flex flex-col items-center">
                <Loader2 className="h-6 w-6 sm:h-8 sm:w-8 animate-spin text-blue-500" />
                <p className="mt-3 sm:mt-4 text-mobile-sm text-gray-600">กำลังเข้าสู่ระบบ...</p>
              </div>
            </div>
          ) : (
            <MobileButton
          ...
          <p className="text-mobile-xs text-gray-400 px-2 leading-relaxed">
            เข้าสู่ระบบเพื่อจัดการงานและผู้สมัครของคุณ
          </p>
        </div>
      </MobileContainer>
    </div>
