...
try {
  ...
  // Add auth state change listener to handle errors
  auth.onAuthStateChanged((user) => {
    // Silently handle auth state changes
  }, (error) => {
    console.warn('⚠️ Firebase Auth state error (non-critical):', error.code);
    // Don't throw error to prevent app crash
  });
  
  // Enable persistence for authentication with error handling
  setPersistence(auth, browserLocalPersistence)
    .then(() => {
      console.log('✅ Firebase Auth persistence enabled');
    })
    .catch((error) => {
      console.warn('⚠️ Failed to enable Firebase Auth persistence (non-critical):', error.code);
      // Don't throw error - persistence is not critical
    });
